                ...
                eventPoint.save(function (err) {
                    ...
                    Event.findOneAndUpdate(
                        ...
                        function (err,event) {
                            ...
                            //if no file or file isn't supported
                            if(!image || !path.extname(image.name) || aws_s3.supportedFileExtensions.indexOf(path.extname(image.name)) === -1){
                                return utils.result(res,code.success,msg.success,event);
                            }
                            /* Temporary location of our uploaded file */
                            var tmp_path = image.path;
                            /* The file name of the uploaded file */
                            imageName = event._id+"_img"+path.extname(image.name);
